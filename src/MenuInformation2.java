...
public class MenuInformation2 extends LogoFix {
	...
	public MenuInformation2 (MainFrame f) {
	    ...
	    btnAdd.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
            	...
            	Menu2.lblList[num].setText(selectedMenu+" ("+selectedSize+" /"+selectedHotIce+" ) x "+count); 
            	Menu2.Array.add(Menu.lblList[num].getText()); 
            	
            	if (selectedMenu == "초코라떼") {
            		price = 3000;
            		Menu2.totalPrice = (price*count)+(sizePay*count);
            		Menu2.sum7 = Menu2.totalPrice;
            	} else if (selectedMenu == "딸기라떼") {
            		price = 3500;
            		Menu2.totalPrice = (price*count)+(sizePay*count);
            		Menu2.sum8 = Menu2.totalPrice;
            	} else if (selectedMenu == "녹차라떼") {
            		price = 3500;
            		Menu2.totalPrice = (price*count)+(sizePay*count);
            		Menu2.sum9 = Menu2.totalPrice;
            	} else if (selectedMenu == "흑당버블티") {
            		price = 4000;
            		Menu2.totalPrice = (price*count)+(sizePay*count);
            		Menu2.sum10 = Menu2.totalPrice;
            	} else if (selectedMenu == "허브티") {
            		price = 3500;
            		Menu2.totalPrice = (price*count)+(sizePay*count);
            		Menu2.sum11 = Menu2.totalPrice;
            	} else if (selectedMenu == "자몽티") {
            		price = 3500;
            		Menu2.totalPrice = (price*count)+(sizePay*count);
            		Menu2.sum12 = Menu2.totalPrice;
            	}
